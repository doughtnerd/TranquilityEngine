...
export default class SceneRenderer {
  static drawFrame(camera, renderQueue) {
    ...
    const cameraTransformationMatrix = mat4.fromRotationTranslation(mat4.create(), quaternion, [
  ...
  static render(renderTarget, renderData, viewMatrix, projectionMatrix) {
    ...
    renderData.material.attributes.forEach((attributeData, index) => {
      renderTarget.bindBuffer(renderTarget.ARRAY_BUFFER, attributeData.buffer);
      ...
      renderTarget.enableVertexAttribArray(renderTarget.getAttribLocation(shaderProgram, attributeData.name));
    });

    {
      renderTarget.uniformMatrix4fv(renderTarget.getUniformLocation(shaderProgram, "uModelMatrix"), false, modelMatrix);

      renderTarget.uniformMatrix4fv(renderTarget.getUniformLocation(shaderProgram, "uModelViewMatrix"), false, modelViewMatrix);

      renderTarget.uniformMatrix4fv(renderTarget.getUniformLocation(shaderProgram, "uViewProjectionMatrix"), false, viewProjectionMatrix);
      ...
      renderTarget.uniformMatrix4fv(renderTarget.getUniformLocation(shaderProgram, "uWorldMatrix"), false, mat4.create());
      ...
      for (const key in renderData.material.textures) {
        ...
        renderTarget.uniform1i(renderTarget.getUniformLocation(shaderProgram, key), index);
      ...
      for (const key in renderData.material.colors) {
        renderTarget.uniform4fv(renderTarget.getUniformLocation(shaderProgram, key), renderData.material.colors[key]);
      }
