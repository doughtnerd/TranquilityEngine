...
class SpriteRenderer extends GameBehavior {
  ...
  awake() {
    ...
    this.material.shader.shaderProgram = this.material.shader.loadShaderProgram(Renderer.glContext);
    this.material.textures.uDiffuse = TextureLoader.load(this.sprite, Renderer.glContext)
    this.material.colors.uDiffuseColor = this.color;

    this.material.attributes.forEach(attribute => {
      attribute.buffer = Material.createBuffer(Renderer.glContext, attribute.data);
    });
  }
